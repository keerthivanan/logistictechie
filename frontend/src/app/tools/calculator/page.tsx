...
import {
    ...
    RefreshCw, TrendingUp, Clock, ShieldCheck,
...
function PortInput({ label, value, onChange, onSelect, placeholder }: {
    ...
    return (
        <div ref={ref} className="relative space-y-1.5">
            <label className="block text-[10px] font-bold text-zinc-600 uppercase tracking-widest font-inter">{label}</label>
            <div className="relative">
                ...
                <input type="text" value={value} autoComplete="off"
                    ...
                    className="w-full bg-black border border-white/5 pl-9 pr-3 py-2.5 rounded-lg text-[10px] font-bold text-white font-inter focus:border-white/20 outline-none tracking-widest"
                />
            </div>
            {open && (options.length > 0 || searching) && (
                <div className="absolute z-50 w-full mt-1 bg-[#0d0d0d] border border-white/10 rounded-2xl overflow-hidden shadow-2xl max-h-56 overflow-y-auto">
                    {searching && <div className="px-4 py-3 text-[9px] text-zinc-600 font-inter font-bold uppercase tracking-widest animate-pulse">Searching Maersk Port Database…</div>}
                    {options.map(p => (
                        <button key={`${p.code}-${p.name}`} type="button"
                            ...
                            <div className="w-7 h-7 rounded-lg bg-white/[0.03] border border-white/5 flex items-center justify-center shrink-0">
                                <span className="text-[7px] font-bold text-zinc-500">{p.country_code}</span>
                            </div>
                            <div>
                                <div className="flex items-center gap-2">
                                    <span className="text-[11px] font-bold text-white font-mono tracking-wider">{p.code}</span>
                                    <span className="text-[9px] text-zinc-400 font-inter">{p.name}</span>
                                </div>
                                <span className="text-[8px] font-bold text-zinc-700 uppercase tracking-widest font-inter">{p.country} · {p.type}</span>
                            </div>
                        </button>
                    ))}
                </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-12 md:gap-20 mb-12">
                        {[{ v: '50+', l: 'Trade Lanes' }, { v: 'Q1 2025', l: 'Market Data' }, { v: 'Live', l: 'Sanctions Check' }].map(s => (
                            <div key={s.l} className="flex flex-col gap-1">
                                ...
                                <div className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] font-bold">{s.l}</div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col gap-12 items-center">
                        ...
                        <div className="w-full space-y-8">
                            <div className="space-y-3">
                                ...
                                <p className="text-[9px] text-zinc-700 font-inter">Indicative only — verify with a licensed freight forwarder before booking.</p>
                            </div>

                            {/* Quick routes */}
                            <div>
                                <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest font-inter mb-3">Quick Routes</p>
                                <div className="flex flex-wrap gap-2">
                                    {QUICK_ROUTES.map(r => (
                                        <button key={`${r.orig}-${r.dest}`} type="button" onClick={() => applyRoute(r)}
                                            className={`px-3 py-1.5 rounded-lg border text-[9px] font-bold font-inter uppercase tracking-wide transition-all ${origCode === r.orig && destCode === r.dest ? 'bg-white text-black border-white' : 'border-white/10 text-zinc-500 hover:border-white/30 hover:text-white'}`}>
                                            {r.orig_n} → {r.dest_n}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <form onSubmit={handleCalculate} className="bg-zinc-950 border border-white/5 rounded-3xl p-8 space-y-5">
                                    ...
                                    <div className="flex rounded-lg border border-white/5 overflow-hidden bg-black">
                                        {['20FT', '40FT', '40HC', '45HC'].map(t => (
                                            <button key={t} type="button" onClick={() => { setContainer(t); setResult(null) }}
                                                className={`flex-1 py-2.5 text-[10px] font-bold uppercase tracking-widest transition-all font-inter ${container === t ? 'bg-white text-black' : 'text-zinc-500 hover:text-white'}`}>
                                                {t.replace('FT', "'")}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-bold text-zinc-600 uppercase tracking-widest font-inter">Commodity</label>
                                    <div className="relative">
                                        <select value={commodity} onChange={e => { setCommodity(e.target.value); setResult(null) }}
                                            className="w-full bg-black border border-white/5 rounded-lg px-3 py-2.5 text-[10px] font-bold text-white font-inter focus:border-white/20 outline-none appearance-none cursor-pointer">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-bold text-zinc-600 uppercase tracking-widest font-inter">Commercial Invoice Value (USD)</label>
                                    <div className="flex bg-black border border-white/5 rounded-lg items-center px-3 focus-within:border-white/20 transition-all">
                                        <span className="text-zinc-600 font-bold text-[10px] uppercase tracking-widest font-inter mr-2">$</span>
                                        <input type="number" value={goodsValue} onChange={e => setGoodsValue(e.target.value)}
                                            className="w-full bg-transparent py-2.5 text-[10px] font-bold text-white font-inter outline-none" />
                                    </div>
                                    <p className="text-[9px] text-zinc-700 font-inter">Used for MPF, duty, and VAT/GST calculations</p>
                                </div>

                                <button type="submit" disabled={loading}
                                    className="w-full bg-white text-black py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-zinc-200 transition-all flex items-center justify-center gap-2 font-inter disabled:opacity-50">
                            ...
                            {result && !result.cannot_ship && (
                                <div className="space-y-4">
                                        ...
                                        <div className="bg-amber-950/30 border border-amber-500/20 rounded-2xl p-5 space-y-2">
                                            {result.warnings.map((w, i) => (
                                                <div key={i} className="flex items-start gap-3">
                                                    ...
                                                    <p className="text-[10px] text-amber-400/80 font-inter leading-relaxed">{w}</p>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <div className="bg-zinc-950 border border-white/5 rounded-3xl p-8 relative overflow-hidden">
                                        ...
                                        <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em] font-inter mb-2">Total Estimated Landed Cost</p>
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest font-inter">Market range:</span>
                                            <span className="text-[10px] font-bold text-zinc-400 font-mono">${result.market_range?.low.toLocaleString()} – ${result.market_range?.high.toLocaleString()}</span>
                                        </div>
                                        <div className="flex flex-wrap gap-6 pt-3 border-t border-white/5">
                                            {[
                                                ...
                                                <div key={x.l}>
                                                    <p className="text-[8px] font-bold text-zinc-700 uppercase tracking-widest font-inter">{x.l}</p>
                                                    <p className="text-[10px] font-bold text-zinc-400">{x.v}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-zinc-950 border border-white/5 rounded-3xl p-6">
                                        <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em] font-inter mb-5">Ocean Freight + Surcharges</p>
                                        <div className="space-y-2.5">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <span className="text-[8px] text-zinc-600 ml-2">(${result.freight?.ocean_low.toLocaleString()}–${result.freight?.ocean_high.toLocaleString()})</span>
                                                </div>
                                            </div>
                                    ...
                                    <div className="bg-zinc-950 border border-white/5 rounded-3xl p-6">
                                        <div className="flex items-center justify-between mb-5">
                                            ...
                                            <span className="text-[8px] font-bold text-purple-400 uppercase tracking-widest bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/20">Destination Customs</span>
                                        </div>
                                        ...
                                        <div className="pt-4 border-t border-white/5 space-y-1.5">
                                            <p className="text-[8px] font-bold text-zinc-700 uppercase tracking-[0.2em] font-inter flex items-center gap-2"><Info className="w-3 h-3" /> Regulatory Notes</p>
                                    ...
                                    <Link href="/search" className="block w-full bg-blue-600 text-white py-3 rounded-lg text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-blue-500 transition-all text-center font-inter shadow-xl">
