        ...
        <div ref={ref} className="relative space-y-1.5">
            ...
            {open && (options.length > 0 || searching) && (
                <div className="absolute z-50 w-full mt-1 bg-[#0d0d0d] border border-white/10 rounded-2xl overflow-hidden shadow-2xl max-h-56 overflow-y-auto">
                    {searching && <div className="px-4 py-3 text-xs text-zinc-600 font-inter animate-pulse">{tPort('calc.searching.ports')}</div>}
                    {options.map(p => (
            ...
            <section className="relative bg-black min-h-screen flex flex-col justify-center">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
                    ...
                    <div className="flex flex-wrap justify-center gap-12 md:gap-20 mb-12">
                        {[{ v: '50+', l: t('calc.trade.lanes') }, { v: 'Q1 2025', l: t('calc.market.data') }, { v: 'Live', l: t('calc.sanctions.check') }].map(s => (
                    ...
                    <button onClick={() => calcRef.current?.scrollIntoView({ behavior: 'smooth' })}
                        ...
                        {t('calc.calculate.rate')} <ArrowRight className="ml-2 w-4 h-4" />
                    </button>
                </div>
            </section>
                    ...
                    <div className="grid md:grid-cols-2 gap-20 items-start">
                        ...
                        <div className="space-y-8">
                            <div className="space-y-4">
                                <h2 className="text-4xl md:text-5xl font-bold tracking-tight">{t('calc.know.cost')}</h2>
                                <p className="text-lg text-zinc-400 leading-relaxed max-w-xl">
                                    {t('calc.desc')}
                                </p>
                                <p className="text-[11px] text-zinc-600 font-inter">{t('calc.indicative')}</p>
                            </div>

                            {/* Quick routes */}
                            <div className="space-y-3 pt-4 border-t border-white/5">
                                <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider font-inter">{t('calc.quick.routes')}</p>
                        ...
                        {/* RIGHT: form + results */}
                        <div className="space-y-4">
                            <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider font-inter mb-5">{t('calc.rate.calculator')}</p>
                            <form onSubmit={handleCalculate} className="bg-zinc-950 border border-white/5 rounded-2xl p-8 space-y-5">
                                <div className="grid grid-cols-2 gap-4">
                                    <PortInput label={t('calc.origin')} value={origCode} placeholder={t('calc.locode.placeholder')}
                                        onChange={v => { setOrigCode(v); setOrigName(v); setResult(null) }}
                                        onSelect={p => { setOrigCode(p.code); setOrigName(p.name); setResult(null) }} />
                                    <PortInput label={t('calc.dest')} value={destCode} placeholder={t('calc.locode.placeholder')}
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-xs font-medium text-zinc-500 font-inter">{t('calc.container.type')}</label>
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-xs font-medium text-zinc-500 font-inter">{t('calc.commodity')}</label>
                                    <div className="relative">
                                        <select value={commodity} onChange={e => { setCommodity(e.target.value); setResult(null) }}
                                            ...
                                            <option value="General" className="bg-zinc-900">{t('calc.com.general')}</option>
                                            <option value="Hazardous" className="bg-zinc-900">{t('calc.com.hazardous')}</option>
                                            <option value="Refrigerated" className="bg-zinc-900">{t('calc.com.reefer')}</option>
                                            <option value="Valuable" className="bg-zinc-900">{t('calc.com.valuable')}</option>
                                            <option value="OOG" className="bg-zinc-900">{t('calc.com.oog')}</option>
                                        </select>
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-xs font-medium text-zinc-500 font-inter">{t('calc.invoice.value')}</label>
                                    ...
                                    <p className="text-[11px] text-zinc-600 font-inter">{t('calc.invoice.help')}</p>
                                </div>
                                ...
                                <button type="submit" disabled={loading}
                                    ...
                                    {loading ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> {t('calc.calculating')}</> : <><RefreshCw className="w-3.5 h-3.5" /> {t('calc.calculate')}</>}
                                </button>
                            </form>
                            ...
                            {result?.cannot_ship && (
                                <div className="bg-red-950/40 border border-red-500/30 rounded-2xl p-8 space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-sm font-bold text-red-400 uppercase tracking-wider font-outfit">{t('calc.cannot.ship')}</h3>
                                    </div>
                            ...
                            {result && !result.cannot_ship && (
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-zinc-950 border border-white/5 rounded-2xl p-8 relative overflow-hidden">
                                        ...
                                        <p className="text-xs font-medium text-zinc-500 font-inter mb-2">{t('calc.total.estimated')}</p>
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            <span className="text-xs text-zinc-600 font-inter">{t('calc.market.range')}</span>
                                        ...
                                        <div className="flex flex-wrap gap-6 pt-3 border-t border-white/5">
                                            {[
                                                { l: t('calc.via'), v: result.lane?.via },
                                                { l: t('calc.transit.label'), v: `${result.lane?.transit_days} days (est.)` },
                                                { l: t('calc.source'), v: 'Drewry / SCFI / Xeneta Q1 2025' },
                                            ].map(x => (
                                                <div key={x.l}>
                                    ...
                                    <div className="bg-zinc-950 border border-white/5 rounded-2xl p-6">
                                        <p className="text-xs font-medium text-zinc-500 font-inter mb-5">{t('calc.ocean.label')}</p>
                                        <div className="space-y-2.5">
                                            <div className="flex justify-between items-center">
                                                <span className="text-xs text-zinc-400 font-inter">{t('calc.ocean.avg')} {result.freight?.container_type})</span>
                                            ...
                                            <div className="flex justify-between items-center">
                                                <span className="text-xs text-zinc-300 font-bold font-inter">{t('calc.total.freight')}</span>
                                    ...
                                    <div className="bg-zinc-950 border border-white/5 rounded-2xl p-6">
                                        <div className="flex items-center justify-between mb-5">
                                            ...
                                            <span className="text-[10px] font-semibold text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/20">{t('calc.dest.customs')}</span>
                                        </div>
                                        <div className="space-y-2.5 mb-5">
                                            ...
                                            <div className="h-px bg-white/5 my-1" />
                                            <div className="flex justify-between items-center">
                                                <span className="text-xs text-zinc-300 font-bold font-inter">{t('calc.total.customs')}</span>
                                                <span className="font-mono text-purple-400 text-xs font-bold">${result.customs?.total.toLocaleString()}</span>
                                            </div>
                                        </div>
                                        <div className="pt-4 border-t border-white/5 space-y-1.5">
                                            <p className="text-xs font-medium text-zinc-600 font-inter flex items-center gap-2"><Info className="w-3 h-3" /> {t('calc.reg.notes')}</p>
            ...
            <section className="py-24 bg-black border-t border-white/5">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="mb-16 text-center">
                        <h2 className="text-xs font-bold uppercase tracking-[0.3em] text-zinc-500">{t('calc.whats.included')}</h2>
                    </div>
                    <div className="grid md:grid-cols-3 gap-12 text-center">
                        {[
                            { title: t('calc.feat1.title'), desc: t('calc.feat1.desc') },
                            { title: t('calc.feat2.title'), desc: t('calc.feat2.desc') },
                            { title: t('calc.feat3.title'), desc: t('calc.feat3.desc') },
                        ].map(f => (
