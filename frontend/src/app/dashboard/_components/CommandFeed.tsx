...
export default function CommandFeed({ activities, title }: { activities: Activity[], title?: string }) {
    ...
    const ACTION_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
        ...
        SIGNUP:             { label: t('act.account.created'),     icon: UserPlus,     color: 'text-white' },
        ...
        MARKETPLACE_SUBMIT: { label: t('act.shipment.requested'),  icon: Store,        color: 'text-white' },
        ...
        BID_SUBMITTED:      { label: t('act.bid.submitted'),       icon: Zap,          color: 'text-white' },
        TASK_CREATED:       { label: t('act.task.created'),        icon: FileText,     color: 'text-zinc-400' },
        TASK_COMPLETED:     { label: t('act.task.completed'),      icon: CheckCircle2, color: 'text-white' },
        TASK_REOPENED:      { label: t('act.task.reopened'),       icon: History,      color: 'text-yellow-400' },
        BOOKING_CREATED:    { label: t('act.booking.created'),     icon: CheckCircle2, color: 'text-white' },
